...
Find below a list of `usdtped` commands added with the `x/vesting` module. You can obtain the full list by using the `usdtped -h` command.
...
```go
usdtped add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
usdtped query vesting balances ADDRESS [flags]
```
...
```go
usdtped tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
usdtped tx vesting clawback ADDRESS [flags]
```
...
```go
usdtped tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
