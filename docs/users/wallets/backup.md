...
```bash
$ usdtped keys add dev0-restored --recover
...
```bash
usdtped keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ usdtped keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ usdtped keys unsafe-import-eth-key dev0-imported ./dev0.export
