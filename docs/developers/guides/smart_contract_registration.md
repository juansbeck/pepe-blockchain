...
usdtped tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
usdtped q revenue contract $CONTRACT
```
...
usdtped tx revenue update $CONTRACT $WITHDRAWER \
...
usdtped tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
