...
Before upgrading the Evmos version. Stop your instance of `usdtped` using `Ctrl/Cmd+C`.
...
::: tip
If the software version does not match, then please check your `$PATH` to ensure the correct `usdtped` is running.
:::
...
```bash
rm $HOME/.usdtped/config/addrbook.json
