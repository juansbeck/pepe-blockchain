...
```bash
usdtped init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .usdtped/config folder

```bash
mv $HOME/genesis.json ~/.usdtped/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.usdtped/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.usdtped/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.usdtped/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.usdtped/config/config.toml
```

### Create usdtped service

```bash
echo "[Unit]
Description=usdtped Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which usdtped) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/usdtped.service; sudo mv $HOME/usdtped.service /etc/systemd/system/
```

```bash
sudo systemctl enable usdtped.service && sudo systemctl daemon-reload
```

### Run usdtped

```bash
sytemctl start usdtped
```

### Check logs

```bash
journalctl -u usdtped -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.usdtped/config/config.toml
```
